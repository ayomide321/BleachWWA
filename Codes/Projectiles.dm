...
mob
	Move()
		if(src.saied||src.neged||src.frozen||src.aoeChancer||src.moving&&!src.allowmove||src.npc||src.tripped||src.nomove&&!src.shunning&&!src.allowmove)
			if(!antiStunner)
				return
			else
				if(src.ko) return
		if(src.antiStunner&&!src.flags) src.rundelay=1
		if(src.client)
			if(src.inshikai||src.inbankai||src.infull||src.inafull||src.ressurection)
...
obj
	ice
		icon = 'Icons/hitsugayashikai.dmi'
		icon_state = ""
		density = 1
		layer = 60
		New()
		...
		Bump(A)
			if(ismob(A))
				...
				if(damage < 1)
					damage =rand(25,1000)
				src.loc=M.loc
				//if(!M.protected)view(O,8) << "<b><font color = red>[O] hit [M] with his wave for [damage] damage!"
...
obj
	freezingmary
		icon = 'Icons/Ice Cast-MissleProjectile.dmi'
		icon_state = ""
		density = 1
		New()
			spawn(15)
...
obj
	SingleNewGetsuga
		icon = 'Icons/32x32 bankai ichigo getsuga.dmi'
		icon_state = "resized"
		density = 1
		single=1
		New()
...
mob
	WingBallHome
		name="Fireball Graveyard"
		icon='Icons/wingBlast.dmi'
		density=0
		rundelay=-50
		ispedal=1
		layer=30
		...
		Move()
			...
			for(var/mob/M in view(1,src))
				if(M.loc==src.loc)
					...
					if(!M.protected)Blood(M.x,M.y,M.z)
					var/mob/D = src.owner
					var/damage = (D.attack+D.reiatsu) - M.defence / 5
					if(damage <= 0)
						damage =rand(25,1000)
					//if(!M.protected)
					//	view(D,8) << "<b><font color = red>[D] hit [M] with his shuriken for [damage] damage!"
		...
		Bump(A)
			if(ismob(A))
				...
				if(M.ispedal)
					src.loc=M.loc
					return
				if(M.bossyama)return
				if(M.bossyama2)return
				if(prob(M.instinct*5))
					var/remember=M.dir
					src.loc=M.loc
					if(src.y==M.y)
...
//				M.rei-=round(M.mrei/2)
	...
	weakcero
		...
		Move()
			...
			..()
	bluefirebarrage
		icon = 'Icons/blue fire barrage.dmi'
		icon_state = "head"
		density = 1
		New()
			spawn(1)
		...
		Bump(A)
			if(ismob(A))
				...
				if(istype(A,/obj/))
					...
					src.loc=T.loc
	baboonblasttrail
		icon = 'Icons/renjibankaimove.dmi'
		layer=30
		New()
			spawn(10)
			src.loc=null
	...
	weakwolf
		...
		Move()
			...
			for(var/mob/M in view(1,src))//get_step(src,src.dir)
				...
				src.loc=null
	BlueFireBlast
		icon='Icons/Blue Fire Blast.dmi'
		density = 1
		New()
			spawn(20)
			src.loc=null
		Bump(A)
			...
			if(istype(A,/mob/player))
				var/mob/M=A
				var/mob/O=src.Gowner
				var/obj/ccerose/k=new/obj/BlueFireBlast1(M.loc)
				k.Gowner=src.Gowner
				if(M == src.Gowner)
					return
				if(M.ispedal)
				...
				if(prob(M.instinct*5))
					...
					if(src.x==M.x)
						...
						if(f==4)step(M,SOUTH);M.dir=remember
					return
				var/damage = O.reiatsu * 5.5- M.defence
				if(prob(7.5))
					if(!M.bluedeboostcd)
						view(8) << "<b>[M] has been dazed! He stumbles and is unable to hit as hard!"
						M.bluedeboostcd=1
						M.reiatsu -= M.mreiatsu*1.2
						M.defence -= M.mdefence*1.2
						M.attack -=	M.attack *1.2
						spawn(120)
							view(8) << "<b>[M] is no longer dazed! They are able to hit at full strength!"
							M.bluedeboostcd=0
							M.reiatsu -= M.mreiatsu*1.2
							M.defence -= M.mdefence*1.2
							M.attack -=	M.attack *1.2
				if(damage < 1)
					damage =rand(100,8000)
				src.loc=null
				//if(!M.protected)view(O,8) << "<b><font color = red>[O] hit [M] with his Cero wolf for [damage] damage!"
				M.Death(O,damage,rand(1,2))
				src.loc=null
		...
		Bump(A)
			...
			if(istype(A,/obj/))
				...
				if(kink.Gowner)
					...
					walk(src,kink.OppositeDirection(),1)
	senkeisword
		icon = 'Icons/senkei.dmi'
		density = 1
		layer=17
		pixel_x=-16
		s1
			icon = 'Icons/senkei1.dmi'
			pixel_x=16
		s2
			icon = 'Icons/senkei2.dmi'
			pixel_x=-16
			pixel_y=32
		s3
			icon = 'Icons/senkei3.dmi'
			pixel_x=16
			pixel_y=32
		New()
...
obj
	arrowrapid
		icon = 'Icons/quincyarrowrapid.dmi'
		icon_state = ""
		density = 1
		single=1
		New()
		...
		Bump(A)
			if(ismob(A))
				...
				if(!M.protected)Blood(M.x,M.y,M.z)
				var/mob/O = src.Gowner
				var/damage = round(O.reiatsu*3.5 - M.defence / 3)
				if(damage < 1)
	...
	arrow
		...
		Bump(A)
			if(ismob(A))
				...
				Blood(M.x,M.y,M.z)
				var/mob/O = src.Gowner
				var/damage = round(src.attack - M.defence / 2.5) // attack /1.25
				if(damage < 1)
					damage =rand(500,1000)
				if(O.blinding)
		...
		Move()
			...
			for(var/obj/s in view(0,src))
				if(s.Gowner && s.sp==1)
					s.sp=2
					var/image/I = image('Icons/silverstands.dmi',s)  //make an image attached to usr
					usr << I
...
obj
	seele
		icon = 'Icons/silverseeleshoot.dmi'
		icon_state = ""
		density = 1
		New()
			spawn(35)
			...
			if(ismob(A))
				...
				if(M.soccer)
					...
					spawn(1)
						...
						if(s.dir==NORTH)s.dir=SOUTH
				var/mob/O = src.Gowner
				var/damage = round((O.reiatsu * 2)+(O.attack*2) - M.defence /1.2)
				if(damage < 1)
					damage =rand(25,1000)
				//if(!M.protected)view(O,8) << "<b><font color = red>[O] hit [M] with his [O.swordname] for [damage] damage!"
				src.loc = M.loc
			...
			if(istype(A,/turf/))
				...
				if(T.density)
					for(var/mob/M in view(2,src))
						var/mob/O = src.Gowner
						O.protected =1
						var/damage = O:attack*35- M.defence / 2
						if(damage < 1)
							damage =rand(25,1000)
						src.loc = M.loc
						O.banmastery(7)
...
obj
 renji1
		icon = 'Icons/renjishikaimove.dmi'
		icon_state = "1"
		New()
			..()
			spawn(4)
...
obj
 mayuripoison
		icon = 'Icons/mayuribankaimove.dmi'
		layer=MOB_LAYER+10
		New()
			..()
			spawn(4)
...
obj
 hit1
		icon = 'Icons/hitsugayabankaimove.dmi'
		icon_state = "1"
		layer=MOB_LAYER+10
		New()
			..()
		...
		Bump(A)
			if(ismob(A))
				...
				if(prob(M.instinct*5))
					...
					return
				var/mob/O = src.Gowner
				var/damage = O.attack * 4.5 - M.defence / 2
				if(damage < 1)
					damage =rand(25,1000)
				if(M.chadref)
					var/K = new/obj/reflection(M.loc)
 ...
 directo
		...
		Bump(A)
			if(ismob(A))
				...
				O.loc = M.loc
				src.loc = locate(0,0,0)
				M.underlays += 'Icons/eldirecto.dmi'
...
obj
	LeftHalTrail2
		layer = 100
		icon = 'Icons/lightning halberd - Copy - Copy.dmi'
