...
obj/FloatingSword
	...
	Bump(A)
		if(ismob(A))
			...
			if(!M.protected)Blood(M.x,M.y,M.z)
			var/mob/O = src.Gowner
			var/damage = round(O.attack *4 - (M.defence/1.4) )
			if(damage < 1)
				damage =rand(25,1000)
			//if(!M.protected)view(O,8) << "<b><font color = red>[O] hit [M] with his desgarron for [damage] damage!"
			src.loc = M.loc
...
mob/proc
	...
	HealerAttack()
		if(HealerAttack==1)
			...
			src.HealerAttack=1
	DecaySounds/Aizen/'WAVE()
		if(src.decwayon)
			src<<"<b>You deactivate your death Sounds/Aizen/'WAVEs."
			src.decwayon=0
		if(src.frozen)
			src<<"Wait!"
			return
		if(!src.decwayon)
			src.decwayon=1
			src<<"<b>You activate your death Sounds/Aizen/'WAVEs!"
mob/proc
	CreateWindBarrier()
		if(src.safe||src.fireback)
			return
