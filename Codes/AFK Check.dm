			...
			for(var/mob/M in All_Clients())
				if(!M.ooctoggle&&M.client)
					if(usr.squad=="Kido Corps")
						M<<output("<font color=purple><b>{[copytext("[src.guildname]",1,4)]}</b></font color><font color=#1736A6><b>([src.status] Kido Corps)<font color=#008CC8>[src]</b></font color>: [msg]","output")
					if(usr.squad!=0&&usr.squad!="Kido Corps")
						M<<output("<font color=purple><b>{[copytext("[src.guildname]",1,4)]}</b></font color><font color=#1736A6><b>([src.status] of Squad [src.squad])<font color=#008CC8>[src]</b></font color>: [msg]","output")
			return
		if(usr.status)
			for(var/mob/M in All_Clients())
				if(!M.ooctoggle&&M.client)
					M<<output("<font color=purple><b>{[copytext("[src.guildname]",1,4)]}</b></font color><font color=#1736A6><b>([src.status])<font color=#008CC8>[src]</b></font color>: [msg]","output")
		if(usr.espadas)
			for(var/mob/M in All_Clients())
				if(!M.ooctoggle&&M.client)
					M<<output("<font color=purple><b>{[copytext("[src.guildname]",1,4)]}</b></font color><font color=#1736A6><b>([src.espadas])<font color=#008CC8>[src]</b></font color>: [msg]","output")
		if(!usr.status&&!usr.espadas)
			for(var/mob/M in All_Clients())
				if(!M.ooctoggle&&M.client)
					M<<output("<font color=purple><b>{[copytext("[src.guildname]",1,4)]}</b></font color><font color=#1736A6><b>(OOC)<font color=#008CC8>[src]</b></font color>: [msg]","output")
