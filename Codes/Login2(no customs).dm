...
mob
	...
	//		MoveThatHatUp()
			...
			/*	if(src.key=="Sasuke13oo9")
				...
				if(src.key=="The Shinigami Ryu")
					...
					if(!src.royshined2)
						...
						src.total_bought+=1000
						src.donor_points+=1000
				if(src.key=="Awesome93" || src.key =="Zagros5000")
				...
				if(src.key =="WSHGC" || src.key == "WorldStar")
					src.verbs|=/mob/CustomZan/verb/Change_To_Ginjo
					src.verbs|=/mob/CustomZan/verb/Vai_Fuse
					src.verbs|=/mob/CustomZan/verb/Special_Zangetsu
					src.verbs|=/mob/CustomZan/verb/Sado_Fuse
		...
		/*			src.contents|=new/obj/skillcard/Acceleration
			...
			//		src.ftype="Ginjo"
				...
				if(src.gotflashboost==1)
					src.flashpause=0
				if(src.key!="Critical172"&&src.key!="Zagros5000"&&src.key!="Dragonpearl123"&&src.key=="Lan0345"||src.key!="Jaffizz"||src.key!="Mexican joker")
				...
				if(src.key=="Zagros5000")
					world<<output("<font size2><font color=white><B>Info:<font color= red> Zagros/Rhyuke(Zagros5000) has Logged in.","output")
				if(src.key=="Mexican joker")
		...
		//			world<<output("<font size2><font color=white><B>Info:<font color= #ff4500> The fox has arrived.","output")
				...
				if(src.race == "Shinigami")
					if(src.squad)
						...
						if(src.status == "Captain Commander")
		...
		/*		if(!src.needrespec||src.needrespec==1||src.needrespec==2)
				...
				if(src.Rebirthedpart2==1)src.curmis="Kill [src.newshikills]/500 shinigamis, [src.newfulkills]/200 fullbringers, [src.newholkills]/500 hollows, [src.newquikills]/200 quincies, [src.newsadkills]/200 sados, [src.newvaikills]/200 vaizards, [src.newpvpkills]/5 player, [src.newvand]/200 vanders"
				...
				if(src.race=="Sado"||src.race=="Fullbringer")Xcution_Ranking(src)
				...
				if(src.squad=="Kido Corps"&&src.status!="Captain Commander")SquadKid_Ranking(src)
...
client
 New()
		...
		//		del(M)
		if(maxplayers>80)
			if(src.key != "Critical172"&&src.key != "Chev45"&&src.key != "Young money 214"&&src.key != "Zagros5000" ||src.key == "Fldaboy"&&src.key != "Prince of cats")
				src.mob.toofull=1
				del src
				return
		maxplayers+=1
