...
/*		TestConnection()
	...
	proc
		...
		GainMoney(amt)
			if(active_fortune)
				amt*=1.5
				src<<"<font color='red'>Good Fortune Bonus!</font>"
			money+=round(amt)
		...
		SpendDonorPoints()
			if(donor_points>0)
				...
				"Free Zans","Vaizard Retake","Black Flames of God","Free Ress","Free Fullbring","Squad Change","Stat Up","Event Points","Exp Burst","Good Fortune","Vasto Form Hollowification","Sado Arm","Boosted")
				switch(i)
					if("2x Level Up")
						if(donor2levels==0)
							if(UseDP(200))
...
/*					if("Eyes of God")
		...
		RetrDP()//query the database to get golden ryo.
			...
			if(isnull(confirm))
				...
				if(confirm=="Yes.")
					alert( src, "Sorry Donor Point system is offline at the moment. Contact Zagros5000 for information regarding Donor Points.")
