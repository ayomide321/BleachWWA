...
turf/EnterMusic
	...
	Entered(mob/M)
		if(M.client)
			if(M.musicStart==0)
				M.musicStart=1
				var/sound/S = sound('YammyFight.ogg',volume=45)
				M << S
				spawn(6000)
					M.musicStart=0
obj/dankaifloor
		...
		if(ismob(A))
			...
			if(M.client)
							...
							for(var/mob/player/Kd in world)
								if(X.dteamid==Kd.dteamid)
									if(X.bosskill ==1)
										...
										Kd.GainExp(Kd.mexp)
										Kd.Level_Up()
										Kd << "<b><font color = lime>Good try, only got 1/3 bosses, but its worth 2 levels. Try harder next time!"
									if(X.bosskill==2)
										Kd.GainExp(Kd.mexp)
										Kd.Level_Up()
										Kd.GainExp(Kd.mexp)
										...
										Kd.GainExp(Kd.mexp)
										Kd.Level_Up()
										Kd << "<b><font color = lime>So close! But you only got 2/3 bosses, but its worth 3 levels. Try a little harder next time!"
									if(X.bosskill==3)
										Kd.GainExp(Kd.mexp)
										Kd.Level_Up()
										Kd.GainExp(Kd.mexp)
										...
										Kd.GainExp(Kd.mexp)
										Kd.Level_Up()
										Kd << "<b><font color = lime>Good Job! Your group got all 3 bosses, Here are 4 levels for the hard work!"
									Kd.bossR1kills=0
									Kd.bossR2kills=0
									Kd.bossR3kills=0
						X.bosskill=0
				...
				if(M.dungeon_healer==1)
					...
					M.contents-=/obj/skillcard/Dungeon_AOE_Heal
		...
		if(ismob(A))
			...
			if(M.client)
				...
				if(M.gotthecharge&&portaltime)
					...
					return
				if(M.level < 850)
					M<<"You do not meet the level requirement of 850, Train some more before entering!"
					return
				if(M.teamid)
				...
				if(M.dteamid==dun1 && M.d1cd==1)
					...
					if(M.dteam_leader == 1)
						if(M.entered == 0)
							M.entered=1
							world << "<b><font color = yellow>[M]'s Dungeon Party has Entered a dungeon! The 40 min cooldown has now begun."
									...
									for(var/mob/player/Kd in world)
										if(X.dteamid==Kd.dteamid)
											if(X.bosskill ==1)
												Kd.GainExp(Kd.mexp)
												Kd.Level_Up()
												Kd << "<b><font color = lime>Good try, only got 1/3 bosses, but its worth 1 levels. Try harder next time!"
											if(X.bosskill==2)
												Kd.GainExp(Kd.mexp)
												Kd.Level_Up()
												Kd.GainExp(Kd.mexp)
												Kd.Level_Up()
												Kd << "<b><font color = lime>So close! But you only got 2/3 bosses, but its worth 2 levels. Try a little harder next time!"
											if(X.bosskill==3)
												Kd.GainExp(Kd.mexp)
												Kd.Level_Up()
												Kd.GainExp(Kd.mexp)
												...
												Kd.GainExp(Kd.mexp)
												Kd.Level_Up()
												Kd << "<b><font color = lime>Good Job! Your group got all 3 bosses,However you failed to exit in time. Here are 3 levels for the effort!"
										Kd.bossR1kills=0
										Kd.bossR2kills=0
										Kd.bossR3kills=0
									X.bosskill=0
...
mob/verb
	...
	Create_Dungeon_Party()
		...
		if(usr.level<850)
			usr << output("<b>You do not meet the level requirement to join or host a dungeon party! Level to at least 850.","output")
			return
...
mob/Dungeon_Party_Leader
	verb
		Dungeon_Team_Invite()
			...
			if(!M)return
			if(M.level<850)
				usr << output("<b>[M]'s level does not meet the requirement of 850.","output")
				return
			if(istype(M,/mob/player))
				switch(alert(M,"Do you want to join [usr]'s Dungeon party?","Join Dungeon Party","Yes","No"))
					if("Yes")
						var/Js = 1
						for(var/mob/player/Md in world)
