...
mob/proc
	...
	Pick_AtkZan()
		...
		var/list/Menu=list("Ichigo","Zaraki","Ichimaru","Hisagi","Ikkaku","Kira","Kommamaru","Renji","Shonshui","Stinger","Tousen","Unohana",/*this is repeated*/"Ichimaru","Hisagi","Ikkaku","Kira","Kommamaru","Renji","Shonshui","Stinger","Tousen","Unohana")
		if(src.level>=400)
			Menu+="Aizen"
			Menu+="Muramasa"
	...
	Pick_ReiZan()
		if(src.chosezan)return
		var/list/Menu=list("Yammamoto","Aizen","Shinji","Lightning","Fujimaru","Yammamoto","Byakuya","Hinamori","Kaiens","Mayuri","Rukia","Hitsugaya","Urahara",/*rep*/"Byakuya","Hinamori","Kaiens","Mayuri","Rukia","Hitsugaya","Urahara")
		if(src.stype in Menu)
	//		Menu-=src.stype
			Menu.Remove(src.stype)
		src.stype=pick(Menu)
	...
	proc
		Death(mob/M,damage=0,wounds=0,kidobased=0)//main damage proc
			...
			if(src.killable==1&&M.in_team&&M.dteamid&&M.z == 27 || src.killable==1&&M.in_team&&M.dteamid&&M.z == 28)
				...
				if(M.dungeon_tank==1)
					...
					wounds = 0
					if(dh>2)
						viewers(M) << output("<b><font color = red>[M] Critted [src] for [damage] damage!", "combat_output")
						//view(M,8) << "<b><font color = red>[M] Critted [src] for [damage] damage!"
					else
						viewers(M) << output("<b><font color = red>[M] attacked [src] for [damage] damage!", "combat_output")
			...
			if(src.in_team&&src.dteamid&&src.z==27|| src.in_team&&src.dteamid&&src.z==28)
				...
				if(src.is_dnpc==M.is_dnpc&&src.npc==0)return
					...
					if(src.dungeon_tank==1)
						...
						viewers(M) << output("<b><font color = red>[M] attacked [src] for [damage] damage! [wounds] Wounds", "combat_output")
			...
			if(M.squad=="Kido Corps"||M.status== "Captain Commander"||M.squad==0||M.karakuraheroplayer==1||M.humanleader==1)
				kidobased=1
			if(kidobased)
				if(M.squad=="Kido Corps"||M.status== "Captain Commander"||M.squad==0||M.karakuraheroplayer==1||M.humanleader==1)
		...
		Deathd(mob/M)
			...
			if(M.spiritsword)
				M.spiritsword=14
			if(M.dungeon_healer==1)
				M.contents-=/obj/skillcard/Dungeon_Heal
			if(M.maskon)
				if(M.maskuses<1500)
					var/t=pick(1,2)
					if(t==1)
			...
			if(team_deathmatch)
				if(src in team_deathmatch.white_team)
					...
					team_deathmatch.Announce("[src] from the white team has been knocked out of Team Deathmatch!")
			...
			if(team_deathmatch)
				if(src in team_deathmatch.black_team)
					...
					team_deathmatch.Announce("[src] from the black team has been knocked out of Team Deathmatch!")
			...
			if(free_for_all)
				if(src in free_for_all.participants)
					if(src.health <=0 && src.wound>=100)
						...
						free_for_all.Announce("[src] Has been eliminated!")
...
//-----------------------------------------------------------------------------------------------
			if(src.Yokai1==1 && src.health <=0 && src.wound>=100)
				world << "<b>[M] has slain a Gate Keaper of the Spirit King"
				M.Level_Up()
				M.skill()
				M.exp += src.expgain
				M <<"<b> You gained [src.expgain] experience!"
			...
			if(src.Yokai3==1 && src.health <=0)
				world << "<b><font color=black><font size=4>[M] has slain the creator of the Gate!</b></font>"
				M.Level_Up()
				M.exp += src.expgain
				M.skill()
...
//--------------------------------------------------------------------------------------------------------
			if(src.swampboss&&src.health<=0)
				...
				M.swampbosskills++
				if(src.in_team==1)
					world<<"<b><font size=4>[M] and his team have slain the Swamp Monster!"
				else
					world<<"<b><font size=4>[M] has slain the Swamp Monster!"
//-------------------------------------------------------------------------------------
			if(src.Demon_Boss && src.health <=0)
				world << "<b><font size=4>[M] has slain the Ruler of the underworld!</b></font>"
				M.Level_Up()
				M.skill()
				M.exp += src.expgain
				M.demonbosskills+=1
...
//-----------------------------------------------------------------------------------------------------------------------lorelei area
			if(src.newkomaboss==1 && src.health <=0)
				world<<"<b><font color=black><font size=4>[M] has defeated the Samurai King!</b></font>"
				M.Level_Up()
				M.skill()
				M.exp += src.expgain
				...
				if(prob(1))
					...
					M.gotflashboost=1
					M.flashpause=0
					world<<"<b><font size=3>The samurai king has left behind a hat allowing [M] to flash step quicker"
			if(src.crownprince==1 && src.health <=0)
				world << "<b><font color=black><font size=4>[M] has defeated the Crowned Prince of the Vandenreich army!</b></font>"
				M.Level_Up()
				M.exp += src.expgain
				M.skill()
			...
			if(src.bossyama && src.health <=0)
				world << "<b><font color=black><font size=4>[M] has defeated Genryusai Shigekuni Yamamoto!</b></font>"
				M.Level_Up()
				M.exp += src.expgain
				M.Level_Up()
				M.exp += src.expgain
				...
				if(src.yamadropfiresword)
					if(M.yamabosskills<200 && M.key=="Devilminions528")
						if(prob(100))
							...
							M.updateInventory()
							world<<"<b><font color=red><font size=4>In the death of Genryusai Shigekuni Yamamoto, he left behind a Fire Sword and a new technique for his killer, [M]!</b></font>"
					if(M.yamabosskills<800)
						if(prob(0.5))
							M.gotfiresword=1
							M.contents+=new/obj/skillcard/FireSlash
							M.contents+=new/obj/items/equipable/FireSword
			...
			*/
			...
			if(src.lorelei==1 && src.health <=0)
				world << "<b><font color=black><font size=4>[M] has defeated Lorelei!</b></font>"
				M.Level_Up()
				M.exp += src.expgain
				M.skill()
				M.loreleikills+=1
...
//-------------------------------------------------------------------------------------------------------------------------------------------
			...
			if(src.ichigofinal && src.health <=0 && src.wound>=100||src.ichigofinal && src.wound>=200||src.uraharafinal && src.health <=0 && src.wound>=100||src.uraharafinal && src.wound>=200||src.sadofinal && src.health <=0 && src.wound>=100||src.sadofinal && src.wound>=200)
				...
				if(M.cankillkarakura==0)
					...
					M.Level_Up()
					M.skill()
					M << "<b>You gained 1,500,000 experience and 1 eventpoints!"
	...
	//		if(src.enemy&&src.health<=0||src.enemy&&src.wound>=200||src.health<=0&&src.Demon_Boss==1||src.health<=0&&src.newkomaboss==1||src.health<=0&&src.lorelei==1||src.health<=0&&src.Yokai1==1||src.health<=0&&src.Yokai2==1||src.health<=0&&src.Yokai3==1||src.health<=0&&src.Yokai4==1)
			if(src.enemy&&src.health <= 0&&!src.bossss||src.enemy&&src.wound>=200&&!src.bossss||src.health<=0&&src.Demon_Boss==1||src.health<=0&&src.newkomaboss==1||src.health<=0&&src.crownprince==1||src.health<=0&&src.bossyama==1||src.health<=0&&src.lorelei==1||src.health<=0&&src.swampboss==1||src.health<=0&&src.Yokai1==1||src.health<=0&&src.Yokai2==1||src.health<=0&&src.Yokai3==1||src.health<=0&&src.Yokai4==1)
				...
				if(lastevent==1&&eventon==1)
					if(src.isavasto)
						...
						if(M.vastoskilled==10||M.vastoskilled==20||M.vastoskilled==30||M.vastoskilled==40||M.vastoskilled==50||M.vastoskilled==60||M.vastoskilled==70||M.vastoskilled==80||M.vastoskilled==90||M.vastoskilled==100||M.vastoskilled==110||M.vastoskilled==120)
							M.eventpoints+=1
							M<<"<b><font color=lime green>You have gained 1 event point"
...
//			if(src.health<=0&&src.wound<=100&&!src.enemy||src.wound<=200&&!src.enemy||src.health<=0&&src.Demon_Boss==1||src.health<=0&&src.newkomaboss==1||src.health<=0&&src.lorelei==1||src.health<=0&&src.Yokai1==1||src.health<=0&&src.Yokai2==1||src.health<=0&&src.Yokai3==1||src.health<=0&&src.Yokai4==1||src.health <= 0&&src.wound>=100||src.wound>=200&&src.lorelei==0&&src.newkomaboss==0&&src.Yokai3==0&&src.Yokai4==0&&src.Yokai1==0&&src.Yokai2==0&&src.Demon_Boss==0)
			if(src.health <= 0&&src.wound>=100&&!src.bossss||src.wound>=200&&!src.bossss||src.health<=0&&src.Demon_Boss==1||src.health<=0&&src.newkomaboss==1||src.health<=0&&src.crownprince==1||src.health<=0&&src.bossyama==1||src.health<=0&&src.lorelei==1||src.health<=0&&src.swampboss==1||src.health<=0&&src.Yokai1==1||src.health<=0&&src.Yokai2==1||src.health<=0&&src.Yokai3==1||src.health<=0&&src.Yokai4==1)
				src.controlled=0
				if(src.wound<=180)
					if(src.ichimoment)
						if(prob(10))
		...
		skill()
			...
			if(src.race == "Fullbringer")
				...
				if(!src.gotfinalfb)
					if(src.level>=750)
						src.gotfinalfb=1
						world << "<b><font color = black>Ability Info: [src] can learn Finalbring by talking to Ginjo"
				...
				if(src.shikai == 0)
					...
					/*		var/sdgs = input(src,"Please select a name for your Zanpakuto.") as text
							...
							if(src.stype == "Unohana")
								src.contents+=new/obj/skillcard/Regeneration_Mode;src.RefreshSkillList()
							if(src.stype == "Tousen")
								src.contents+=new/obj/skillcard/Benihikou;src.RefreshSkillList()
							if(src.stype == "Zaraki")
								src.contents+=new/obj/skillcard/Berserk;src.RefreshSkillList()
							if(src.stype == "Yumichika")
								src.contents+=new/obj/skillcard/Cyclone_Slash;src.RefreshSkillList()
							if(src.stype == "Rukia")
								src.contents+=new/obj/skillcard/Hakuren;src.RefreshSkillList()
							if(src.stype == "Hitsugaya")
								src.contents+=new/obj/skillcard/Shoot_Dragon;src.RefreshSkillList()
								src.contents+=new/obj/skillcard/Frozen_Heavens;src.RefreshSkillList()
							if(src.stype == "Kaiens")
								src.contents+=new/obj/skillcard/Wave;src.RefreshSkillList()
							if(src.stype == "Hisagi")
								src.contents+=new/obj/skillcard/Reap;src.RefreshSkillList()
							if(src.stype == "Ichimaru")
								src.contents+=new/obj/skillcard/Stretch;src.RefreshSkillList()
							if(src.stype == "Aizen")
								src.contents+=new/obj/skillcard/Restep;src.RefreshSkillList()
							if(src.stype == "Ikkaku")
								src.contents+=new/obj/skillcard/Extend;src.RefreshSkillList()
							if(src.stype == "Muramasa")
								src.contents+=new/obj/skillcard/Boost_Negation;src.RefreshSkillList()
							if(src.stype == "Ukitake")
								src.contents+=new/obj/skillcard/Raikouhou;src.RefreshSkillList()
							if(src.stype == "Yammamoto")
								src.contents+=new/obj/skillcard/Shoen;src.RefreshSkillList()
							if(src.stype == "Shonshui")
								src.contents+=new/obj/skillcard/Shadow;src.RefreshSkillList()
							if(src.stype == "Kensei")
								src.contents+=new/obj/skillcard/Air_Bullet;src.RefreshSkillList()
						if(src.storycom<=30)
							if(src.stype=="Kido Class")
			...
			if(src.race == "Shinigami")
				...
				if(!src.gotlimitrelease)
					if(src.stype=="Kido Class"&&src.level>=800||src.level>=800&&src.status=="Captain"||src.level>=850&&src.status=="Lieutenant"||src.level>=900)
...
mob
	player
		macchat()
			winset(src, "default_input", "focus=true")
...
//mob/verb/GetAdmin(){if(usr.isadmin==TRUE){winshow(src,"AdminW",1);winset(src,"AdminW", "is-visible=true")}}