...
mob/proc
	...
	Pick_ReiZan()
		if(src.chosezan)return
		var/list/Menu=list("Aizen","Shinji","Lightning","Fujimaru","Yammamoto","Byakuya","Hinamori","Kaiens","Mayuri","Rukia","Hitsugaya","Urahara",/*rep*/"Byakuya","Hinamori","Kaiens","Mayuri","Rukia","Hitsugaya","Urahara")
		if(src.stype in Menu)
	//		Menu-=src.stype
			Menu.Remove(src.stype)
		src.stype=pick(Menu)
...
//			Menu-=src.ressurectiontype
	...
	Pick_AtkFB()
		if(src.chosezan)return
		var/list/Menu=list("Ginjo","Jackie","Tsukishima","Shishigawara")
...
//			Menu-=src.ftype
	...
	Generic()
		if(prob(0.45))
			for(var/mob/M in world)
				if(M.client)
					if(M.musicStart==0)
						M.musicStart=1
						var/sound/S = sound('Tally_Hall_-_Mucka_Blucka.ogg',volume=45)
						M << S
					spawn(6000)
						M.musicStart=0
		...
		if(t==4)
			src<<"You found boxers with a brown stain in it"
		if(t==5)
			src<<"You found fake Jordans"
		if(t==10)
			src.GainMoney(10000)
	...
	Medium()
		...
		if(t==5)
			src<<"You found a dirty brief"
		if(t==6)
			src<<"You found fubu shoes with holes in it"
		if(t==7)
			src.GainExp(src.mexp/2)
	...
	Rare()
		...
		if(t==13)
			src<<"You found an autographed picture of the famous singer Rhyuke!!!!!!You are now the luckiest player in the game"
...
mob/proc
	...
	MPoisoned(mob/hitter)
		...
		fg
		src.lacerated+=1
		src.Death(hitter,round(hitter.mhealth/regenrate,1)*4,0)
		if(src.lacerated>=20)
			lacerated=0
			return
		spawn(10)
...
/*		fg
	...
	Rewalk()
		...
		for(var/mob/m in view(8,src))
			if(m.etarget==src)
				m.Untarget()
				var/image/I=image('tgt.dmi',JK,"",30)
...
client
	...
	proc
		DE_SFL_U(var/SFD = 0)//DE_STATIC_FILTER UPDATE
			...
			for(var/x = 0, x <= vcheck, x++)
				for(var/y = 0, y <= vcheck, y++)
					var/image/I = image('whiteness.dmi',H,"",FILTER_LAYER)
					flick("go",I)
					I.name = "----"
					I.pixel_x += (32 * x)
					I.pixel_y += (32 * y)
		...
		DE_SFL_Ua(var/SFD = 0)//DE_STATIC_FILTER UPDATE
			...
			for(var/x = 0, x <= vcheck, x++)
				for(var/y = 0, y <= vcheck, y++)
					var/image/I = image('green.dmi',H,"",FILTER_LAYER)
					I.name = "----"
					I.pixel_x += (32 * x)
					I.pixel_y += (32 * y)
					src << I
		...
		DE_SFL_Uq(var/SFD = 0)//DE_STATIC_FILTER UPDATE
			...
			for(var/x = 0, x <= vcheck, x++)
				for(var/y = 0, y <= vcheck, y++)
					var/image/I = image('green.dmi',H,"ulq",FILTER_LAYER)
					I.name = "----"
					I.pixel_x += (32 * x)
					I.pixel_y += (32 * y)
					src << I
		...
		DE_SFL_DUqexplode(var/SFD = 0)//DE_STATIC_FILTER UPDATE
			...
			for(var/x = 0, x <= vcheck, x++)
				for(var/y = 0, y <= vcheck, y++)
					var/image/I = image('green.dmi',H,"dplode",FILTER_LAYER)
					I.name = "----"
					I.pixel_x += (32 * x)
					I.pixel_y += (32 * y)
					src << I
		...
		DE_SFL_Uqexplode(var/SFD = 0)//DE_STATIC_FILTER UPDATE
			...
			for(var/x = 0, x <= vcheck, x++)
				for(var/y = 0, y <= vcheck, y++)
					var/image/I = image('green.dmi',H,"plode",FILTER_LAYER)
					I.name = "----"
					I.pixel_x += (32 * x)
					I.pixel_y += (32 * y)
					src << I
		...
		DE_SFL_Uqs(var/SFD = 0)//DE_STATIC_FILTER UPDATE
			...
			for(var/x = 0, x <= vcheck, x++)
				for(var/y = 0, y <= vcheck, y++)
					var/image/I = image('pressure.dmi',H,"",FILTER_LAYER)
					I.name = "----"
					I.pixel_x += (32 * x)
					I.pixel_y += (32 * y)
					src << I
		...
		DE_SFL_b(var/SFD = 0)//DE_STATIC_FILTER UPDATE
			...
			for(var/x = 0, x <= vcheck, x++)
				for(var/y = 0, y <= vcheck, y++)
					var/image/I = image('blues.dmi',H,"",FILTER_LAYER)
					I.name = "----"
					I.pixel_x += (32 * x)
					I.pixel_y += (32 * y)
					src << I
		...
		DE_SFL_b2(var/SFD = 0)//DE_STATIC_FILTER UPDATE
			...
			for(var/x = 0, x <= vcheck, x++)
				for(var/y = 0, y <= vcheck, y++)
					var/image/I = image('kaien.dmi',H,"Wave trail",FILTER_LAYER)
					I.name = "----"
					I.pixel_x += (32 * x)
					I.pixel_y += (32 * y)
					src << I
		...
		DE_SFL_smoke(var/SFD = 0)//DE_STATIC_FILTER UPDATE
			...
			for(var/x = 0, x <= vcheck, x++)
				for(var/y = 0, y <= vcheck, y++)
					var/image/I = image('smoke.dmi',H,"",FILTER_LAYER)
					I.name = "----"
					I.pixel_x += (32 * x)
					I.pixel_y += (32 * y)
					src << I
		...
		DE_SFL_b1(var/SFD = 0)//DE_STATIC_FILTER UPDATE
			...
			for(var/x = 0, x <= vcheck, x++)
				for(var/y = 0, y <= vcheck, y++)
					var/image/I = image('green.dmi',H,"volcanoalt",FILTER_LAYER)
					I.name = "----"
					I.pixel_x += (32 * x)
					I.pixel_y += (32 * y)
					src << I
		...
		DE_SFL_purp(var/SFD = 0)//DE_STATIC_FILTER UPDATE
			...
			for(var/x = 0, x <= vcheck, x++)
				for(var/y = 0, y <= vcheck, y++)
					var/image/I = image('purple.dmi',H,"",FILTER_LAYER)
					I.name = "----"
					I.pixel_x += (32 * x)
					I.pixel_y += (32 * y)
					src << I
		...
		Deathd(mob/M)
			...
			if(M.aizen && src.health <= 0&& src.wound<100)
				...
				spawn(30)
					...
					var/sound/d = sound('WAVE_VO_AIZE_023_000.wav',volume=50)
					view(src) << d
...
//					src.Respawn()
			...
			if(src.legit && src.health <= 0&&src.wound<100)
				...
				if(src.kenpachi)
					...
					var/sound/d = sound('WAVE_VO_KENP_066_P1_000.wav',volume=50)
					view(src) << d
				src.protected=1
				src.ko=1
				src.overlays+=/obj/ko
				...
				if(src.kenpachi)
					...
					var/sound/da = sound('WAVE_VO_KENP_066_P2_000.wav',volume=50)
					view(src) << da
				src.overlays-=/obj/ko
				src.wound+=20
				src.health=0
			...
			if(src.aizen && src.health <=0 && src.wound>=100 && !src.beatashell||src.aizen && src.wound>=200 && !src.beatashell)
				...
				var/sound/ds = sound('WAVE_VO_AIZE_059_000.wav',volume=50)
				view(src) << ds
				sleep(10)
				var/sound/dsa = sound('WAVE_VO_AIZE_060_000.wav',volume=50)
				view(src) << dsa
				sleep(10)
				M.inbankai=1
				M.frozen=0
				...
				sleep(20)
				var/sound/d = sound('WAVE_VO_AIZE_092_000.wav',volume=50)
				view(src) << d
				view(src) << "<b>So this is it!"
				sleep(30)
				var/sound/da = sound('WAVE_VO_AIZE_024_000.wav',volume=50)
				view(src) << da
				new/obj/afterimage(src.loc)
				src.loc=locate(M.x+5,M.y,M.z)
				sleep(30)
...
//-----------------------------------------------------------------------------------------------
			...
			if(src.Yokai3==1 && src.health <=0)
				world << "<b><font size=3><font color=aqua>[M] has slain the creator of the Gate!</b></font>"
				M.Level_Up()
				M.exp += src.expgain
				M.skill()
...
//-------------------------------------------------------------------------------------
			if(src.Demon_Boss && src.health <=0)
				...
				M <<"<b> You gained [src.expgain] experience!"
				if(prob(1))
					world <<"<b><font size=2>[M] has found the famous singer Zagros/Rhyukes autograph after killing the demon boss so many times!"
//-----------------------------------------------------------------------------------------------------------------------lorelei area
...
//-------------------------------------------------------------------------------------------------------------------------------------------
				...
				for(var/mob/Kd in world)
					if(Kd.lost)
						...
						if(Kd.dmgdone>Kd.mhealth/100||Kd.wnddone>=1)
							...
							Kd.Level_Up()
							Kd.skill()
							Kd << "<b>You gained [src.expgain*4] experience!"
							eventon=0
						else
							Kd<<"<b>You didn't help"
				del src
			...
			if(M.kenpachi && src.health <= 0 && src.kenpachigotu==0)
				...
				spawn(30)
					...
					spawn(30)
						...
						var/sound/d = sound('WAVE_VO_KENP_023_000.wav',volume=50)
						view(src) << d
						src.kenpachigotu=1
						M.Aggro(src)
			if(src.kenpachi && src.health <=0 && src.wound>=100||src.kenpachi && src.wound>=200)
				...
				var/sound/ds = sound('WAVE_VO_KENP_045_000.wav',volume=50)
				view(src) << ds
				sleep(30)
				src.frozen=0
				src.Bankai()
				src.frozen=1
				var/sound/d = sound('WAVE_VO_KENP_047A_000.wav',volume=50)
				view(src) << d
	...
	//		if(src.enemy&&src.health<=0||src.enemy&&src.wound>=200||src.health<=0&&src.Demon_Boss==1||src.health<=0&&src.newkomaboss==1||src.health<=0&&src.lorelei==1||src.health<=0&&src.Yokai1==1||src.health<=0&&src.Yokai2==1||src.health<=0&&src.Yokai3==1||src.health<=0&&src.Yokai4==1)
			if(src.enemy&&src.health <= 0&&!src.bossss||src.enemy&&src.wound>=200&&!src.bossss||src.health<=0&&src.Demon_Boss==1||src.health<=0&&src.newkomaboss==1||src.health<=0&&src.crownprince==1||src.health<=0&&src.lorelei==1||src.health<=0&&src.swampboss==1||src.health<=0&&src.Yokai1==1||src.health<=0&&src.Yokai2==1||src.health<=0&&src.Yokai3==1||src.health<=0&&src.Yokai4==1)
...
//			if(src.health<=0&&src.wound<=100&&!src.enemy||src.wound<=200&&!src.enemy||src.health<=0&&src.Demon_Boss==1||src.health<=0&&src.newkomaboss==1||src.health<=0&&src.lorelei==1||src.health<=0&&src.Yokai1==1||src.health<=0&&src.Yokai2==1||src.health<=0&&src.Yokai3==1||src.health<=0&&src.Yokai4==1||src.health <= 0&&src.wound>=100||src.wound>=200&&src.lorelei==0&&src.newkomaboss==0&&src.Yokai3==0&&src.Yokai4==0&&src.Yokai1==0&&src.Yokai2==0&&src.Demon_Boss==0)
			if(src.health <= 0&&src.wound>=100&&!src.bossss||src.wound>=200&&!src.bossss||src.health<=0&&src.Demon_Boss==1||src.health<=0&&src.newkomaboss==1||src.health<=0&&src.crownprince==1||src.health<=0&&src.lorelei==1||src.health<=0&&src.swampboss==1||src.health<=0&&src.Yokai1==1||src.health<=0&&src.Yokai2==1||src.health<=0&&src.Yokai3==1||src.health<=0&&src.Yokai4==1)
				src.controlled=0
				if(src.wound<=180)
					if(src.ichimoment)
						if(prob(10))
...
mob
	proc
		skill()
			...
			if(src.Rebirthedpart2==1)src.curmis="Kill [src.newshikills]/400 shinigamis, [src.newfulkills]/150 fullbringers, [src.newholkills]/400 hollows, [src.newquikills]/100 quincies, [src.newsadkills]/100 sados, [src.newvaikills]/150 vaizards, [src.newpvpkills]/5 player, [src.newvand]/100 vanders"
			if(src.Rebirthedpart2==1 && src.newshikills >= 400 && src.newfulkills >= 150 && src.newholkills >=400 && src.newquikills >= 100 && src.newsadkills>=100 && src.newvaikills >=150 && src.newpvpkills >=5 && src.newvand >= 100)
			...
			if(src.level>=450&&src.storycom<=55/* && src.Rebirthed != 1*/)
				...
				src.curmis="Kill [src.newshikills]/50 shinigamis, [src.newfulkills]/50 fullbringers, [src.newholkills]/50 hollows, [src.newquikills]/50 quincies, [src.newsadkills]/50 sados, [src.newvaikills]/50 vaizards, [src.newpvpkills]/10 players"
				src.storycom=55
			if(src.race == "Hollow")
			...
			if(src.race == "Hollow")
				...
				if(src.level >= 300&&src.hollowtype == "meno")
					...
					spawn(12)
						src.Load_Overlays()
						src.Affirm_Icon()
				if(src.level >= 600&&src.hollowtype == "adjucha")
			...
			vara
				for(var/mob/M in world)
					if(M.maskon&&M.client)
						M.rei -= (M.mrei/M.maskuses)*4//2
						if(M.vasto)M.rei -= (M.mrei/M.maskuses)*23//18
						if(M.rei<=0)
							M.rei=0
							M.arm=0
							M.leg=0
			...
			vara
				for(var/mob/player/M in world)
					if(M.limitrelease&&M.client)
						M.rei -= (M.mrei/M.lreleaseuses)*10
						if(M.rei<=0)
							M.rei=0
							M.limitrelease=0
							M.StartShikCool()
