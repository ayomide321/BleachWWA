		...
		/*		if(src.gotshishiskills==0)
			...
			if(src.ftype=="Water Doll")
				...
				if(!src.gotwaterdollskills)
					...
					src.contents+=new/obj/skillcard/WaterSounds/Aizen/'WAVE
			...
			if(src.ftype=="Fire Doll")
				...
				if(!src.gotfiredollskills)
					...
					src.contents+=new/obj/skillcard/Heat_Sounds/Aizen/'WAVE
			...
			if(src.ftype=="Samurai")
				if (src.key == "Awesome93" || src.key=="Lan0345"||src.key=="CoreBreaker")
					src.reiatsu+= round(src.mreiatsu*2.2,1)
					src.Load_Overlays()
				src.attack += round(src.mattack*1.5,1)
			...
			if(src.ftype=="Ichigo")
				...
				src.attack += round(src.mattack*1.6,1)
				src.reiatsu += round(src.mreiatsu*1.6,1)
			...
			if(src.ftype=="Tsukishima")
				src.attack += round(src.mattack*2.1,1)
				src.defence += round(src.mdefence*1.5,1)
				if(src.thits>=200)
					src.attack += round(src.mattack*1.6,1)
				if(!src.gothits)
			...
			if(!src.inafull)
				if(src.infull)
					...
					if(src.ftype == "Ichigo")
						...
						src.defence += round(src.mdefence*2,4)
						if(src.gotaccelfb==0)
						...
						src.Load_Overlays()
					if(src.ftype=="Water Doll")
						src.reiatsu += round(src.mreiatsu*4.8,1)
						src.rundelay=1
						src.Load_Overlays()
					if(src.ftype=="Samurai")
						if(src.key == "Awesome93" || src.key =="Lan0345"||src.key=="CoreBreaker")
							src.reiatsu +=round(src.mreiatsu*3.1,1)
							src.Load_Overlays()
					...
					if(src.ftype=="Shishigawara")
						...
						src.Load_Overlays()
					if(src.ftype == "Tsukishima")
						src.attack += round(src.mattack*2.7,1)
						src.defence+= round(src.mdefence*2,1)
						src.Load_Overlays()
					if(src.ftype == "Yukio")
