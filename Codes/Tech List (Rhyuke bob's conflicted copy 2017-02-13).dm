	...
	proc
		...
		Shadow()
			...
			if(!src.shadowcd)
				...
				Kad.Gowner=src
				var/image/Idd = image('Shadow.dmi',Kad)  //make an image attached to src
				src << Idd
	...
	//			return
		...
		Sense()
			...
			for(var/mob/player/M in world)
				if(!M.hiding||src.GM>4||src.key=="Sexytime_explosion")
					Menu.Add(M)
				if(src.squad==3)
					if(M.z==src.z)
						Menu.Add(M)
		...
		Healing()
			...
			if(src.npccaptain&&!src.pet)
				for(var/mob/Gotei_13/M in world)
		...
		Hollow_Screech()
			if(!src.screech)
				...
				for(var/mob/M in oview(8,src))
					if(src.rei > M.rei)
						M.Slowed(10,200)
						var/sound/S = sound('009D_0000.wav',volume=50)
						M << S
						if(M.client)
							M.client.DE_SFL_Uqs()
							spawn(15)
	...
	proc
		...
		Materialize()
			...
			if(src.materializedcool)
				src<<"<b>You must wait at least 10 seconds before materializing again"
				return
			if(src.inshikai||src.inbankai)
				src.rei-=500
				src.shimastery(5)
	...
	//		if(src.wound>=25)
		...
		Sprenger_Pentagon()
			...
			if(!src.sprengerpentagoncd)
				...
				K.Gowner=src
				K.sp=1
				var/image/I = image('silverstand.dmi',K)  //make an image attached to src
				src << I
				var/obj/Ka=new/obj(locate(src.x-2,src.y+1,src.z))
				Ka.Gowner=src
				Ka.sp=1
				var/image/Id = image('silverstand.dmi',Ka)  //make an image attached to src
				src << Id
				var/obj/Kad=new/obj(locate(src.x+2,src.y+1,src.z))
				Kad.Gowner=src
				Kad.sp=1
				var/image/Idd = image('silverstand.dmi',Kad)  //make an image attached to src
				src << Idd
				var/obj/Kac=new/obj(locate(src.x-2,src.y-3,src.z))
				Kac.Gowner=src
				Kac.sp=1
				var/image/Idc = image('silverstand.dmi',Kac)  //make an image attached to src
				src << Idc
				var/obj/Kads=new/obj(locate(src.x+2,src.y-3,src.z))
				Kads.Gowner=src
				Kads.sp=1
				var/image/Idds = image('silverstand.dmi',Kads)  //make an image attached to src
				src << Idds
		...
		Invisibility()
			...
			if(src.rei>=1000)
				...
				for(var/mob/m in oview(4,src))
					if(m.etarget==src)
						m.Untarget()
						var/image/I=image('tgt.dmi',M,"",30)
				...
				spawn(20)
					...
					for(var/mob/m in view(8,src))
						if(m.etarget==src)
							m.Untarget()
							var/image/I=image('tgt.dmi',Ma,"",30)
							I.name="TargetUnder";src.client<<I
...
//				return
		...
		Power_Transfer()
			...
			if(!src.stolen)
				var/mob/M=src.etarget
				src.stolenr=min(18000,M.reiatsu-M.mreiatsu)
				src.stolena=min(18000,M.attack-M.mattack)
				src.stolend=min(18000,M.defence-M.mdefence)
				src.reiatsu+=src.stolenr
				src.attack+=src.stolena
				src.defence+=src.stolend
	...
	proc
		Garganta()
			...
			if(src.wound>50)
				...
				return
			if(src.traveltime)
				src<<"<b>You have to wait [(src.traveltime)] seconds"
				return
			if(!src.firing)
				src.frozen=1
				src.traveltime=150
	...
	proc
		...
		Coffin()
			if(src.rei < 2500)
				...
				return
			if(src.coffincool)
				src<<"<b>You have to wait at least 45 secs"
				return
			if(src.safe||src.fireback)
				return
			if(src.firing)
		...
		Reiatsu_Blast()
			if(src.rei < 100)
				src << "<b>Your rei is too low [src.rei]/100!"
				return
			if(src.safe||src.fireback)
				return
	...
	proc
		Flash_Step()
			if(src.flashstep)
				...
				if(src.race=="Arrancar"||src.race=="Numero"||src.race=="Espada"||src.race=="Sado"||src.race=="Chad")
								...
								spawn(1)
									...
									spawn(1)
										...
										if(Ks)
											...
											walk(Ksss,Ksss.dir)
										spawn(1)
											for(var/obj/Darkban/m in world)
												if(m.Gowner==src)
													walk(m,src.dir)
													spawn(7)
													walk(m,0)
								...
								spawn(1)
									...
									spawn(1)
										...
										if(Ks)
											...
											walk(Ksss,Ksss.dir)
										spawn(1)
											for(var/obj/rukiaban/m in world)
												if(m.Gowner==src)
													walk(m,src.dir)
													spawn(7)
													walk(m,0)
...
/*		Cero_Obscura()
		...
		Sounds/Aizen/'WAVE()
			...
			if(src.rei <= 1000)
				src << "<b>Your rei is too low!"
				return
			if(!src.Sounds/Aizen/'WAVEcd)
				src.Sounds/Aizen/'WAVEcd = 1
				src.rei -= 1000
				if(src.dir == SOUTH||src.dir == NORTH)
					var/obj/cascada/A = new /obj/newkaeinSounds/Aizen/'WAVE/
					var/obj/cascada/B = new /obj/newkaeinSounds/Aizen/'WAVE/
					var/obj/cascada/C = new /obj/newkaeinSounds/Aizen/'WAVE/
					A.loc = locate(src.x, src.y, src.z)
					C.loc = locate(src.x+1, src.y, src.z)
					B.loc = locate(src.x-1, src.y, src.z)
					A.layer=3
					...
					walk(C,src.dir)
					walk(B,src.dir)
					var/obj/cascada/Ca = new /obj/newkaeinSounds/Aizen/'WAVE/
					var/obj/cascada/Ba = new /obj/newkaeinSounds/Aizen/'WAVE/
					Ca.loc = locate(src.x+2, src.y, src.z)
					Ba.loc = locate(src.x-2, src.y, src.z)
					Ba.dir = src.dir
					Ca.dir = src.dir
					...
					walk(Ca,src.dir)
					walk(Ba,src.dir)
					var/obj/cascada/Caa = new /obj/newkaeinSounds/Aizen/'WAVE/
					var/obj/cascada/Baa = new /obj/newkaeinSounds/Aizen/'WAVE/
					Caa.loc = locate(src.x+3, src.y, src.z)
					Baa.loc = locate(src.x-3, src.y, src.z)
					Baa.dir = src.dir
					Caa.dir = src.dir
					...
					walk(Caa,src.dir)
					walk(Baa,src.dir)
					var/obj/cascada/Caaa = new /obj/newkaeinSounds/Aizen/'WAVE/
					var/obj/cascada/Baaa = new /obj/newkaeinSounds/Aizen/'WAVE/
					...
					walk(Caaa,usr.dir)
					walk(Baaa,usr.dir)
					var/obj/cascada/Caaaa = new /obj/newkaeinSounds/Aizen/'WAVE/
					var/obj/cascada/Baaaa = new /obj/newkaeinSounds/Aizen/'WAVE/
		...
		Sovereign_Sounds/Aizen/'WAVE()
			...
			if(src.rei <= 1000)
				src << "<b>Your rei is too low!"
				return
			if(!src.sovereignSounds/Aizen/'WAVEcd)
				src.sovereignSounds/Aizen/'WAVEcd = 1
				src.rei -= 1000
				if(src.dir == SOUTH)
				...
				if(src.dir == WEST||src.dir == NORTHWEST||src.dir == SOUTHWEST||src.dir == EAST||src.dir == NORTHEAST||src.dir == SOUTHEAST)
					...
					walk(Baa,src.dir)
			sleep(230)
			src.sovereignSounds/Aizen/'WAVEcd = 0
...
mob
	proc
		Bala()
			if(src.race=="Arrancar"||src.race=="Yammy Boss")
				if(src.rei < 200)
					src << "<b>Your rei is too low!"
					return
				if(src.safe||src.fireback)
