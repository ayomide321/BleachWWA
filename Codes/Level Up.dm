...
mob/proc/Level_Up()
	...
	//	src.level ++
		...
		if(src.donor4levels!=1&&src.donor2levels==1||src.key == "Silver9014"||src.key == "Oksim7"||src.key == "Anbukev"||src.key == "KillManiac"||src.key == "Fatboy12"||src.key == "Quay400"||src.key == "Ilaero"||src.key == "AzureRain"||src.key == "Icedpali"||src.key == "DevilKnightSparda"||src.key == "Tarcioblazek"||src.key == "Condemned pizz4"||src.key == "Knives221"||src.key == "Vash-Da-Trigun"||src.key == "lunar0011"||src.key == "Jackdriver123"||src.key == "BlackUchiha777"||src.key == "Dornamu"||src.key == "Adlofs"||src.key == "Dexter1"||src.key == "GohantheKid"||src.key == "Anbutyler18"||src.key == "BlowItOutOfYourAss"||src.key == "Goodtooth"||src.key == "Stone186"||src.key == "Xsinx3"||src.key == "WinterSnow")//host funders and myself lol
		...
		if(src.donorhalfexp==1||src.key=="Fatboy12")
			src.mexp-=(src.mexp)/2
		if(src.level == 50)
