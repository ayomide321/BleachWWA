	...
	verb
		Attack()
			...
			if(src.attack>=src.reiatsu)
				damage = round(src.attack/1.5)
			if(src.defence>=(src.attack&&src.reiatsu))
				damage = round(src.defence/1.5)
			...
			if(src.canattack)
				...
				for(var/mob/M in get_step(src,src.dir))
					if(M.shunko||M.tshunko)
						if(prob(M.quickflash*15))
							flick("shun",M)
							if(src.swordon)flick("Sword Slash1",src)
				...
				if(src.stype == "Hyosuke")
					if(src.decwayon)
						...
						src.canattack=1
						return
				if(src.ressurectiontype=="Healer")
				...
				if(src.ressurection==1)
					if(src.ressurectiontype=="Cirucci")
				...
				if(src.stype == "Zaraki"||src.stype == "Ikkaku"||src.stype=="Ichigo"&&src.inshikai||src.stype=="Aizen" && src.inshikai||src.stype=="Aizen" && src.inbankai)
					...
					src.canattack = 1
					return
				if(src.stype=="Rangiku" && src.inshikai||src.stype=="Rangiku" && src.inbankai)
					flick("Sword Slash1",src)
						...
						if(M in src.smoked)
							if(M.smoking)
								...
								if(!M.protected)
									...
									damage=rand(damage*1.1,damage*1.9)
								if(damage <= 0)
									damage = round(src.attack * 1.9 - M.defence / 3)
					...
					return
				if(src.inshikai)
					if(src.stype == "Jiroubou" && src.inshikai)
						src.shurikened+=1
						flick("punch",src)
				...
				if(src.stype=="Shusuke")
					...
					step(K,src.dir)
					step(K,src.dir)
				if(src.stype == "Kira" && src.inbankai)
					flick("Sword Slash1",src)
					...
					src.canattack = 1
					return
				if(src.stype == "Byakuya" && src.inbankai&&src.safezone||src.stype == "Byakuya" && src.inshikai&&src.safezone)
					flick("punch",src)
					if(src.dir==EAST)
					...
					src.canattack=1
					return
				if(src.stype == "Hisagi" && src.inbankai||src.stype == "Hisagi" && src.inshikai)
					...
					src.canattack=1
					return
				if(src.stype == "Kaiens" && src.inbankai||src.stype == "Kaiens" && src.inshikai)
					...
					src.canattack=1
					return
				if(src.stype == "Hinamori" && src.inbankai)
					if(src.rei <= 150)
						src << "<b>Your rei is too low!"
						sleep(7)
						src.canattack=1
				...
				if(src.stype=="Sword Multiplier")
					...
					src.canattack=1
					return
				if(src.stype == "Jiroubou" && src.inbankai)
					src.shurikened+=1
					flick("punch",src)
					var/K = new/mob/shuriken(usr.loc)
					K:owner = usr
				...
				if(damage <= 1)
					damage =rand(25,1000)
				if(src.stype == "Mayuri"&&src.inbankai)
					if(src.rei <= 75)
						src << "<b>Your rei is too low!"
						sleep(6)
						src.canattack=1
					...
					src.canattack = 1
					return
				if(src.stype=="Stinger"&&src.inshikai)
					flick("punch",src)
					for(var/mob/M in get_step(src,src.dir))
						if(M.volcano)
							M.Volcano(src)
				...
				if((src.infull||src.inafull)&&src.ftype=="Wings")
					goto wing1
				if(src.stype=="Kensei"&&src.inbankai)
					flick("punch",src)
					for(var/mob/M in get_step(src,src.dir))
						if(M.volcano)
							M.Volcano(src)
				...
				if(src.stype=="Kensei"&&(src.inshikai||!src.inbankai))
					...
					return
				wing2
				if(src.stype=="Renji"&&src.inbankai)
					src.extending=1
				...
				if(src.newkomaboss==1)
					...
					src.canattack=1
					return
				if(src.inshikai&&src.stype == "Kommamaru"&&src.newkomaboss==0)
					flick("Sword Slash1",src)
					var/obj/k=new/obj/craters/smoke(src.loc)
					k.Gowner=src
					walk(k,src.dir)
				...
				if(src.inbankai&&src.stype == "Kommamaru"&&src.newkomaboss==0)
					damage=src.attack*1.1
					damage+=damage*3
				...
				if(src.ikkakushikai)
					...
					src.canattack = 1
					return
				if(src.stype == "Shonshui"&&src.inshikai||src.stype == "Ukitake"&&src.inshikai||src.stype == "Ichinose"&&src.inshikai&&src.shikaidrain < 10)
					flick("Sword Slash1",src)
					for(var/mob/M in get_step(src,src.dir))
						if(M.volcano)
							M.Volcano(src)
						...
						if(prob(src.stunpassive*3))
							...
							M.Freeze(12)
						if(M.killable)src.shimastery(3)
				if(src.stype=="Shinji"&&src.inshikai||src.stype=="Shinji"&&src.inbankai)
					flick("Sword Slash1",src)
					if(src.inshikai)
						src.shimastery(3)
					if(src.inbankai)
					...
					for(var/mob/M in get_step(src,src.dir))
						...
						src.canattack=1
						return
				if(src.stype == "Yammamoto" && src.inshikai||src.stype == "Yammamoto" && src.inbankai)
					flick("Sword Slash1",src)
					for(var/mob/M in get_step(src,src.dir))
						if(M.volcano)
							M.Volcano(src)
						...
						src.canattack=1
						return
				if(src.stype == "Shonshui"&&src.inbankai||src.stype == "Ukitake"&&src.inbankai)
					flick("Sword Slash1",src)
					damage=damage*3
					for(var/mob/M in get_step(src,src.dir))
						if(M.volcano)
							M.Volcano(src)
						if(src.stype=="Ukitake")
							damage = round(src.reiatsu*2)
							damage+=rand(damage/1.2,damage/1.5)
							damage+=src.swordD
						if(damage <= 1)
							damage =rand(25,1000)
						spawn()M.Death(src,damage,rand(0,1))
				...
				if(!src.renjishikai&&!src.ikkakushikai)
					if(src.swordon)flick("Sword Slash1",src)
					if(!src.swordon)flick("punch",src)
					if(src.stype=="Ichigo"&&src.inbankai)
					...
					for(var/mob/M in get_step(src,src.dir))
						...
						if(src.stype=="Muramasa")
							damage += damage
						if(src.race=="Fullbringer")
							damage=src.attack*1.3+src.mreiatsu*1.3
						//	damage=damage*2
						if(src.race=="Sado"||src.race=="Chad")
							damage=src.attack*1.3
							damage = damage*3.9
						if(src.inbankai&&src.stype == "Byakuya"&&src.senkei||src.ressurectiontype=="Halibel"||src.stype == "Rukia"||src.stype=="Dark Bat"||src.ftype=="Dark Bat")
							damage += src.reiatsu*5.5
						if(src.ftype=="Wings"||src.ressurection&&src.ressurectiontype=="Shooter"||src.ressurection&&src.ressurectiontype=="Pantera"||src.stype == "Hitsugaya"||src.stype == "Urahara"||src.ftype=="Ichigo"||src.stype == "Yumichika"&&src.inbankai||src.stype == "Yumichika"&&src.inshikai)
							damage+=round(src.reiatsu*2)
						...
						if(src.stype=="Stinger"&&src.inbankai||src.ressurectiontype=="Tijereta"&&src.ressurection)
							damage+=damage*1.2
						if(src.ressurectiontype=="Nell"&&src.ressurection)
							damage+=damage*2.6
						if(src.ressurectiontype=="Scientist"&&src.ressurection)
							damage+=damage*2
						if(src.stype=="Fujimaru"&&src.inshikai)
							damage+=src.reiatsu*2
						...
						if(src.ftype=="Yukio"||src.ftype=="Ginjo")
							damage+=(src.reiatsu+src.attack)*1.5
						if(src.inshikai&&src.stype == "Mayuri")
							damage = src.reiatsu *1.5
							damage=rand(damage*0.9,damage*1.2)
						if(src.infinal)
							damage=(src.attack+src.defence+src.reiatsu)*1.5
							damage=rand(damage*0.9,damage*1.2)
						damage -= M.defence / 2
						if(M.stype == "Ikkaku")
						...
						if(src.stype=="Urahara")
							...
							if(src.inbankai&&src.bankaidrain > 0)
								if(M.killable)src.banmastery(4)
						if(src.stype=="Tousen"&&src.inshikai||src.stype == "Tousen" && src.inbankai)
							var/randblindness=pick(0,1)
							damage += damage
							if(randblindness==1)
								M.Blindyou(10)
								damage=rand(damage*0.9,damage*2)
						...
						if(src.ressurectiontype=="Tijereta")
							...
							Blood(M.x,M.y,M.z)
							spawn()M.Death(src,damage,rand(2,8))
							...
							if(SpearThrow == 0)
								...
								if(src.ulqgotsword)
									...
									for(M in get_step(src,src.dir))
										...
										src.canattack = 1
										return*/
						if(src.stype == "Rukia" || src.stype=="Dark Bat" || src.ftype=="Dark Bat")
							if(src.inshikai == 1||src.inbankai == 1||src.infull || src.inafull)
								if(M)
									if(src.stype == "Dark Bat" || src.ftype=="Dark Bat")
										src.random = rand(1,75)
										if(src.random <= 25&&!M.protected)
											damage = round(src.reiatsu/1.5)
											damage+=rand(damage/2,damage/1.5)
				...
				//		walk(B,src.dir)
						...
						src.canattack = 1
						return
					if(src.stype=="Hitsugaya"&&src.inbankai)
						damage = round(src.reiatsu*1.5)
						damage+=rand(damage/2,damage/1.5)
						if(damage <= 1)
							damage =rand(25,1000)
				...
				if(src.crownprince||src.iswandevent)
					...
					M:Gowner = src
					walk(M,src.dir)
				if(src.stype == "Renji"&&src.inshikai)
					src.frozen = 1
					src.extending=1
					damage+=damage*2
