...
#warn Implement new rebirth/prestige system sometime
	...
	Owner
		verb
			RebirthRelapse1(mob/M in world)
				set category = "Owner"
				switch(input(M,"Hello, are you here to Prestige or get your Prestige rewards??","Ending", text) in list ("Prestige","Rewards"))

					if("Rewards")
						if(!M.Rebirthed123)
							M<<"Gotta rebirth first."
							return
						if(M.gotrewards)
							M<<"Already claimed."
							return
					...
					if("Prestige")
						AutoSave()
						M.talkingtorb = 1
						switch(input(M,"Are you really really sure you want to Prestige, last chance!?","Prestige", text) in list ("Yes","No"))
							if("No")
								AutoSave()
								M.talkingtorb = 1
								return
